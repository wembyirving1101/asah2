...
function layoutGraph(source: SkillNode[], edges: SkillEdge[]): { nodes: PositionedNode[]; connections: SkillEdge[] } {
  ...
  parents.forEach((parent) => {
    ...
    branchChildren.forEach((child, index) => {
      ...
      const point = direction === 'up' ? { x: center.x + offset, y: center.y + 130 } : direction === 'down' ? { x: center.x + offset, y: center.y + 130 } : direction === 'left' ? { x: center.x - 220, y: center.y + offset } : { x: center.x + 220, y: center.y + offset }
